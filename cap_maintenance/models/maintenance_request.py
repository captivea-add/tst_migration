...
def yield_file_paths_from_directory_path(directory_path):
    ...
    """
    for entry in os.scandir(path=directory_path):
        if entry.is_file():
            yield entry.path
...
class MaintenanceRequest(models.Model):
    ...
    def compute_has_attachment_recovery_directory(self):
        ...
        for record in self:
            ...
            if (record.attachment_recovery_directory_path
                    and all(os.path.isdir(directory_path)
                            for directory_path in record.attachment_recovery_directory_path.split(","))):
                value = True
    ...
    #Profile
    user_profile = fields.Selection([('Hitachi', 'Hitachi'),
                                     ('Partenaire', 'Partenaire'),
                                     ('Client', 'Client'),
                                     ('Autre','Autre')],
                                    compute='compute_user_profile', default=lambda self: self.env.user.x_studio__stu)

    internal_usage = fields.Boolean(string='Internal use')
    company_type = fields.Selection([('TOSA', 'Related to TOSA'),
                                     ('OTHER', 'Related to Puissance')], string="Company Type",
                                    related='company_id.company_type')

    # Sous-ensemble
    @api.depends_context('force_company')
    def _subgroup_domain(self):
        domains = "('company_id', 'in', [company_id, False])"
        if self.env.company == self.env.ref('cap_settings.company_tosa'):
            domains += ", ('equipment_group_id', '=', equipment_group)"
        return "[{}]".format(domains)

    def _default_mailing_list(self):
        if self.env.company.company_type == 'TOSA':
            return [(6, 0, [self.env.user.partner_id.id])]
        else:
            return [(6,0,[])]

    equipment_subgroup = fields.Many2one(string='Subgroup', comodel_name='maintenance.equipment.subgroup', check_company=True, domain=lambda self: self._subgroup_domain())

    # Intervenant ABB / Intervenant sur site
    worker_ids = fields.Many2many(string='Workers', comodel_name='res.partner', relation='maintenance_request_worker_rel', domain="[('x_studio_profil', '!=', 'Autre'), ('company_id', 'in', [company_id, False])]")
    # Responsable
    manager_id = fields.Many2one(string='Manager', comodel_name='res.users', required=True, default=lambda self: self.env.user, check_company=True,  domain="[('x_studio__stu', 'in', ['Hitachi']), ('company_id', 'in', [company_id, False])]")
    ...
    mailing_list_final_report_partner_ids = fields.Many2many(string='Mailing List Final Report',
                                                             ...
                                                             check_company=True,
                                                             default=lambda self: self._default_mailing_list()
                                                             )
    ...
    # Autres Documents
    # -------------------------------------------------------------------
    other_document_ids = fields.Many2many(comodel_name='ir.attachment',
                                          relation='maintenance_request_other_document_ids_rel',)
    ...
    satisfied_customer_organization = fields.Boolean('Satisfied Customer')
    comments_organization = fields.Text('Comments about organization')
    satisfied_customer_intervention = fields.Boolean('Satisfied Customer')
    comments_intervention = fields.Text('Comments about intervention')
    customer_maintenance_plan = fields.Boolean('Maintenance Plan Applied by the Customer')
    domain_maintenance_plan = fields.Text('Application Domain')
    frequency_maintenance_plan = fields.Text('Frequency')
    comments_maintenance_plan = fields.Text('Comments about maintenance plan')
    # ---------------------------------------------------------------------------------------------------------
    #                                       FIELDS CALCULATION
    # ---------------------------------------------------------------------------------------------------------
    def compute_user_profile(self):
        for record in self:
            record.user_profile = self.env.user.x_studio__stu
    ...
    def onchange_manager_id(self):
        ...
        for record in self:
            ...
            if record.manager_id.partner_id not in record.mailing_list_final_report_partner_ids:
                record.mailing_list_final_report_partner_ids += record.manager_id.partner_id
    ...
    def create_attachment_from_file_path(self, file_path):
        ...
        args = {"name": os.path.basename(file_path).encode("utf-8", "ignore").decode("utf-8"),
